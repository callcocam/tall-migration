...
*/
namespace Tall\Migration\Definitions;

use Illuminate\Support\Str;
use Tall\Migration\Helpers\ValueToString;
use Tall\Migration\Helpers\WritableTrait;

/**
 * Class ColumnDefinition
 * @package Tall\Migration\Definitions
 */
class ColumnDefinition
{
    use WritableTrait;
    ...
    protected function guessLaravelMethod()
    {
        ...
        if (config('tall-migration.definitions.prefer_unsigned_prefix') && $this->unsigned) {
            return [$this->columnName, 'uuid', []];
    ...
    public function render(): string
    {
        ...
        if ($this->index) {
            $indexName = '';
            if (count($this->indexDefinitions) === 1 && config('tall-migration.definitions.use_defined_index_names')) {
        ...
        if ($this->primary && ! $this->isPrimaryKeyMethod($finalMethodName)) {
            $indexName = '';
            if (count($this->indexDefinitions) === 1 && config('tall-migration.definitions.use_defined_primary_key_index_names')) {
        ...
        if ($this->unique) {
            $indexName = '';
            if (count($this->indexDefinitions) === 1 && config('tall-migration.definitions.use_defined_unique_key_index_names')) {
                $indexName = ValueToString::make($this->indexDefinitions[0]->getIndexName());
            }
        ...
        if ($this->comment !== null && config('tall-migration.definitions.with_comments')) {
